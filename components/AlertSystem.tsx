  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // 0. Determine Species based on Latitude
        const species = getRegionBearInfo(userLat);
    ...
    <div className="h-full w-full bg-white/80 backdrop-blur-xl rounded-3xl shadow-lg border border-white/60 p-6 flex flex-col justify-between">
      <div className="space-y-4">
        ...
        {envRisk && (
           <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              {envRisk.weather ? (
                <div className={`p-4 rounded-2xl border flex flex-col gap-1 shadow-sm transition-all duration-500 animate-in fade-in ${
                   envRisk.weather.riskModifier === 'HIGHER' 
              ...
              {speciesInfo && (
                <div className={`col-span-1 md:col-span-2 p-4 rounded-2xl border flex items-start gap-3 shadow-sm animate-in fade-in slide-in-from-bottom-2 ${
                  speciesInfo.type === 'BROWN' 
                    ? 'bg-amber-900/5 border-amber-900/10' 
                    : 'bg-stone-900/5 border-stone-900/10'
                }`}>
                    <div className={`p-2 rounded-xl shrink-0 ${
                      speciesInfo.type === 'BROWN' ? 'bg-amber-900 text-white' : 'bg-stone-800 text-white'
                    }`}>
                      <PawPrint size={20} />
                    </div>
                    <div className="flex-1 min-w-0">
                       <div className="flex justify-between items-start">
                          ...
                          <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${
                            speciesInfo.type === 'BROWN' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-800'
                          }`}>
        ...
        {alertLevel !== AlertLevel.NONE && nearestSpot && distance !== null ? (
          <div className={`rounded-2xl p-4 border animate-in slide-in-from-bottom-2 duration-500 ${
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`font-bold text-base mb-0.5 ${
                  alertLevel === AlertLevel.DANGER ? 'text-red-800' : 'text-slate-800'
                }`}>
      ...
      <div className="mt-4">
        ...
        <button
          ...
          disabled={loading}
          className={`w-full py-3.5 px-6 rounded-xl flex items-center justify-center gap-2 font-bold transition-all duration-300 shadow-lg hover:shadow-xl active:scale-95 text-sm ${
            loading
